  ...
  export function MilkRecords() {
    ...
    useEffect(() => {
      ...
      console.log("cattleData is here:", cattleData)
      console.log("cattle:", cattle)
      if (Array.isArray(cattleData) && cattleData.length > 0) {
        console.log("First cattle object keys:", Object.keys(cattleData[0]))
      }
    }, [accessToken, cattleData, cattle])
        ...
        <main className="flex-1 p-8">
          ...
          <div className="bg-white rounded-lg border overflow-hidden">
            <Table>
              <TableHeader>
                <TableRow>
                  {["Cattle ID", "Date", "Morning (L)", "Afternoon (L)", "Night (L)", "Total (L)"].map((header, i) => (
                    <TableHead key={i}>
